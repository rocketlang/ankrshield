...
import type { AttackChain, AttackType, ThreatEvent } from '../types';
  ...
  private extractAffectedAssets(events: ThreatEvent[]): string[] {
    ...
    for (const e of events) {
      ...
      if (['NETWORK_UPLOAD', 'NETWORK_REQUEST'].includes(e.action)) {
        try {
          const url = new URL(e.resource.startsWith('http') ? e.resource : `https://${e.resource}`);
  ...
  private buildFallbackActions(attackType: AttackType): string[] {
    ...
    switch (attackType) {
      ...
      case 'honeypot_triggered':
        return [
          'Quarantine agent immediately — honeypot access indicates malicious intent',
          ...base,
        ];
      default:
        return base;
