...
const NARRATION_FALLBACK: NarrationResult = {
  narrative:
    'Suspicious AI agent activity was detected. Review the event log for details.',
  technicalSummary:
    'LLM narration unavailable — see raw event list for technical details.',
  severity: 'warning',
  attackType: 'unknown',
  affectedAssets: [],
  suggestedActions: [
...
export class ThreatNarrator {
  ...
  async narrate(chain: AttackChain): Promise<AttackChain> {
    const agentName =
      chain.events.find((e) => e.agentName)?.agentName ?? 'Unknown Agent';
