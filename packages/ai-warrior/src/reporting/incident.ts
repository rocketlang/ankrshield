...
import type { AttackChain, IncidentReport, ReporterInput, TimelineEntry } from '../types';
...
export class IncidentReporter {
  ...
  private buildRecommendations(input: ReporterInput): string[] {
    ...
    const credentialThefts = input.attackChains.filter((c) => c.attackType === 'credential_theft');
    ...
    const exfiltrations = input.attackChains.filter((c) => c.attackType === 'data_exfiltration');
