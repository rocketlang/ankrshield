...
export class ServiceManager {
  ...
  isHealthy(): boolean {
    ...
    return (
      database?.status === ServiceStatus.RUNNING &&
      user?.status === ServiceStatus.RUNNING
    );
  }
  ...
  private async initializeNetworkService(): Promise<void> {
    ...
    try {
      ...
      this.setServiceStatus('network', ServiceStatus.DEGRADED, 'Initialization failed', error as Error);
  ...
  private async initializePrivacyService(): Promise<void> {
    ...
    try {
      ...
      this.setServiceStatus('privacy', ServiceStatus.DEGRADED, 'Initialization failed', error as Error);
  ...
  private setServiceStatus(
    ...
    const state: ServiceState = {
      ...
      message,
      error,
      lastStarted: status === ServiceStatus.RUNNING ? new Date() : this.services.get(serviceName)?.lastStarted,
      lastStopped: status === ServiceStatus.STOPPED ? new Date() : this.services.get(serviceName)?.lastStopped,
    };
