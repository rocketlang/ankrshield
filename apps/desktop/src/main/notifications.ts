...
export class NotificationService {
  ...
  showProtectionStatus(enabled: boolean): void {
    this.show(
      ...
      enabled ? 'ankrshield is now protecting your privacy' : 'Privacy protection has been paused',
      { urgency: 'normal' }
    );
  }
  ...
  showUpdateAvailable(version: string): void {
    this.show('Update Available', `Version ${version} is available. Downloading...`, {
      urgency: 'normal',
    });
  }
  ...
  showUpdateDownloaded(version: string): void {
    this.show('Update Ready', `Version ${version} has been downloaded. Restart to install.`, {
      urgency: 'normal',
      timeoutType: 'never',
    });
  }
  ...
  showError(message: string): void {
    this.show('Error', message, { urgency: 'critical' });
  }
