...
import { usePrivacyScore, useNetworkStats, useDnsStats, useTopTrackers, useIsLoading, useError } from '../stores/appStore';
    ...
    <div className="p-8 space-y-6">
        ...
        <Card className="bg-gradient-to-br from-gray-800 to-gray-900">
          <div className="text-center">
            <div className="flex items-center justify-center gap-3 mb-4">
              <div className="text-7xl font-bold text-ankr-green">
                {privacyScore.totalScore}
              </div>
              <div className="text-left">
                <div className="text-sm text-gray-400 uppercase tracking-wide">Privacy Score</div>
                <Badge variant={privacyScore.totalScore < 30 ? 'success' : privacyScore.totalScore < 60 ? 'warning' : 'danger'}>
                  {privacyScore.level}
                </Badge>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 pt-6 border-t border-gray-700">
              <div>
                <div className="text-2xl font-semibold text-ankr-blue">{privacyScore.networkScore}</div>
                <div className="text-sm text-gray-400">Network</div>
              </div>
              <div>
                <div className="text-2xl font-semibold text-ankr-purple">{privacyScore.dnsScore}</div>
                <div className="text-sm text-gray-400">DNS</div>
              </div>
              <div>
                <div className="text-2xl font-semibold text-ankr-orange">{privacyScore.appScore}</div>
              ...
              {topTrackers.map((tracker, index) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <Badge
                      variant={
                        tracker.riskScore >= 80 ? 'danger' :
                        tracker.riskScore >= 60 ? 'warning' :
                        'success'
                      }
                    >
      ...
      {!networkStats && !dnsStats && !error && !isLoading && (
        <Alert variant="info" title="No Data Yet">
          Network monitoring will begin capturing data shortly. Make some network requests to see activity.
        </Alert>
      )}
    </div>
  );
