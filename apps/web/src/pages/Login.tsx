/**
 * Login Page — wired to ANKR SSO (port 4260)
 *
 * Supports:
 *   - Email + password
...
export default function Login() {
  ...
  const handlePasswordLogin = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const tokens = await sso.login(email, password);
      storeAndGo(tokens);
    } catch (err) {
  ...
  const handleSendOtp = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await sso.sendOtp(email);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-gray-900 flex items-center justify-center px-4">
      <div className="max-w-md w-full">
        ...
        {/* Tab switcher (only on initial screens) */}
        {mode !== 'otp-verify' && (
          <div className="flex bg-gray-800 rounded-lg p-1 mb-4 border border-gray-700">
            <button
              ...
              className={`flex-1 py-2 rounded-md text-sm font-medium transition-colors ${
                mode === 'password' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-gray-300'
              }`}
            >
              Password
            </button>
            <button
              ...
              className={`flex-1 py-2 rounded-md text-sm font-medium transition-colors ${
                mode === 'otp-send' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-gray-300'
              }`}
            >
              Email Code
            </button>
