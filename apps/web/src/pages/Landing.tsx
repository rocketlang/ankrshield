...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] text-white font-sans antialiased">
      ...
      <section id="github-actions" className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-24">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            {/* Left — copy */}
            <div>
              <Badge color="green">Developer-First</Badge>
              <h2 className="text-4xl font-black text-white mt-4 mb-4">
                Fail your CI pipeline
                <br />
                <span className="text-green-400">before a risky deploy ships.</span>
              </h2>
              <p className="text-gray-400 mb-6">
                One line of YAML adds xShield to every pull request. If the domain risk score
                exceeds your threshold, the merge is blocked — automatically.
              </p>

              <ul className="space-y-3 mb-8">
                {[
                  { icon: '⚡', text: 'Zero setup — works on ubuntu, macos, windows runners' },
                  { icon: '🔑', text: 'Secure — API key stored as a GitHub secret, never logged' },
                  {
                    icon: '📋',
                    text: 'Rich step summary — score, level, and report link in the CI UI',
                  },
                  { icon: '🎯', text: 'Configurable threshold — set your own pass/fail score' },
                  { icon: '📤', text: 'Outputs — pipe score into Slack, Jira, or custom scripts' },
                ].map((item) => (
              ...
              <div className="flex flex-wrap gap-3">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 bg-white text-black font-bold px-5 py-2.5 rounded-xl text-sm hover:bg-gray-100 transition"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 border border-white/20 text-white font-semibold px-5 py-2.5 rounded-xl text-sm hover:bg-white/10 transition"
                >
                  API Docs →
                </a>
              </div>
...
jobs:
  risk-scan:
    ...
    steps:
      - uses: `}
                    <span className="text-yellow-400">actions/checkout@v4</span>
                    {`

      - name: `}
                    <span className="text-green-400">xShield domain risk scan</span>
                    {`
        id: xshield
        uses: `}
                    <span className="text-cyan-400">rocketlang/xshield-scan@v1</span>
                    {`
        with:
          domain: `}
                    <span className="text-orange-400">'yourdomain.com'</span>
                    {`
          api-key: `}
                    <span className="text-purple-400">{`\${{ secrets.XSHIELD_API_KEY }}`}</span>
                    {`
          fail-threshold: `}
      ...
      - name: Print report link
        ...
        run: |
          echo "Score: `}
                    <span className="text-purple-400">{`\${{ steps.xshield.outputs.risk-score }}`}</span>
                    {`"
          echo "Level: `}
                    <span className="text-purple-400">{`\${{ steps.xshield.outputs.risk-level }}`}</span>
                    {`"`}
                  </code>
                </pre>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-[#0d1117] overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-3 border-b border-white/10 bg-white/[0.03]">
                  <span className="text-xs text-gray-500">📋 GitHub Step Summary</span>
                </div>
                <div className="p-5 text-sm">
                  <p className="font-bold text-white mb-3">
                    🟢 xShield Risk Scan — <code className="text-cyan-400">yourdomain.com</code>
                  </p>
                  <div className="border border-white/10 rounded overflow-hidden text-xs font-mono">
                    <div className="grid grid-cols-2 border-b border-white/10">
                      <span className="px-3 py-1.5 text-gray-500 border-r border-white/10">
                        Risk Score
                      </span>
                      <span className="px-3 py-1.5 text-green-400 font-bold">12 / 100</span>
                    </div>
                    <div className="grid grid-cols-2 border-b border-white/10">
                      <span className="px-3 py-1.5 text-gray-500 border-r border-white/10">
                        Risk Level
                      </span>
                      <span className="px-3 py-1.5 text-green-400">MINIMAL</span>
                    </div>
                    <div className="grid grid-cols-2">
                      <span className="px-3 py-1.5 text-gray-500 border-r border-white/10">
                        Threshold
                      </span>
                      <span className="px-3 py-1.5 text-gray-300">70</span>
                    </div>
                  </div>
                  <p className="text-green-400 text-xs mt-3">
                    ✅ PASSED — Risk score 12 is within the threshold of 70.
                  </p>
                </div>
              </div>
            </div>
